...
public class Inventory : MonoBehaviour, ISaveManager
{
    ...
    [SerializeField] private GameObject canCraftUI;
    private ItemSlot_UI[] inventroyItemSlot;
    ...
    private void Start()
    {
        ...
        inventroyItemSlot = inventorySlotParent.GetComponentsInChildren<ItemSlot_UI>();
    ...
    public bool CanCraft(ItemData_Equipment _itemToCraft, List<InventoryItem> _requiredMaterials)
    {
        ...
        for (int i = 0; i < _requiredMaterials.Count; i++)
        {
            if (stashDictionary.TryGetValue(_requiredMaterials[i].data, out InventoryItem stashValue))
            {
                ...
                {
                    Debug.Log("没有足够材料数量");
                    canCraftUI.GetComponentInChildren<TextMeshProUGUI>().text = "没有足够材料";
                    canCraftUI.SetActive(true);
            ...
            else if (inventoryDictionary.TryGetValue(_requiredMaterials[i].data, out InventoryItem inventoryValue))
            {
        ...
        Debug.Log("获得物品：" + _itemToCraft.name);
        canCraftUI.GetComponentInChildren<TextMeshProUGUI>().text = "获得物品：" + _itemToCraft.name;
        canCraftUI.SetActive(true);
        AudioManager.instance.PlaySFX(7);
        return true;
    }
    ...
    {
        for (int i = 0; i < inventroyItemSlot.Length; i++)
        {
            inventroyItemSlot[i].CleanUpSlot();
        }
        ...
        {
            inventroyItemSlot[i].UpdateSlot(inventory[i]);
        }
    ...
    public void UseFlask()
    {
        ...
        else { Debug.Log("药水CD: " + (lastTimeUsedFlask + currentFlask.itemCooldown - Time.time).ToString("f2")); }
    }
    ...
    #region 往各个库存 添加/移除 物品
    public bool CanAddItem()
    {
        if (inventory.Count >= inventroyItemSlot.Length)
        {
            return false;
        }
        return true;
